<script>
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			errorMessage = err instanceof Error ? err.message : 'Failed to sign in. Please try again.';
		} finally {
			isSubmitting = false;
		}
	};
...
<!-- <svelte:head> handles the document head metadata dynamically -->
<svelte:head>
	<title>Sign in — Ledger</title>
	<meta name="description" content="Log in to your Ledger business financial dashboard." />
	<meta property="og:title" content="Sign in — Ledger" />
	<meta property="og:description" content="Log in to your Ledger business financial dashboard." />
</svelte:head>

<div class="flex min-h-screen flex-col bg-background font-body-md text-on-surface">
	<!-- Main Content Canvas -->
	<main class="relative flex grow items-center justify-center overflow-hidden py-xl px-gutter">
		...
		>
			<div class="mb-xl text-center">
				...
				<h2 class="mb-xs font-headline-lg text-headline-lg tracking-tight text-on-surface">
					Welcome Back
				</h2>
				<p class="font-body-md text-body-md text-on-surface-variant">
					Log in to your business financial dashboard
				</p>
			</div>
			...
			<!-- Svelte uses standard lower-case event directives (onsubmit) -->
			...
			<div class="mt-xl text-center">
				<p class="font-body-md text-body-md text-on-surface-variant">
					...
					<a
						...
						href={resolve('/auth/registration')}>Sign up</a
					>
				</p>
			</div>
		</div>
